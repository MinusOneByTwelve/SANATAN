...
-- Create default user "GUACAMOLEADMIN" with password "GUACAMOLEADMIN"
INSERT INTO guacamole_entity (name, type) VALUES ('GUACAMOLEADMIN', 'USER');
...
SELECT
    entity_id,
    @salt,
    UNHEX(SHA2(CONCAT('GUACAMOLEPWD', HEX(@salt)), 256)),
    CURRENT_TIMESTAMP
...
FROM (
          SELECT 'GUACAMOLEADMIN'  AS username, 'CREATE_CONNECTION'       AS permission
    UNION SELECT 'GUACAMOLEADMIN'  AS username, 'CREATE_CONNECTION_GROUP' AS permission
    UNION SELECT 'GUACAMOLEADMIN'  AS username, 'CREATE_SHARING_PROFILE'  AS permission
    UNION SELECT 'GUACAMOLEADMIN'  AS username, 'CREATE_USER'             AS permission
    UNION SELECT 'GUACAMOLEADMIN'  AS username, 'CREATE_USER_GROUP'       AS permission
    UNION SELECT 'GUACAMOLEADMIN'  AS username, 'ADMINISTER'              AS permission
) permissions
...
FROM (
          SELECT 'GUACAMOLEADMIN' AS username, 'GUACAMOLEADMIN' AS affected_username, 'READ'       AS permission
    UNION SELECT 'GUACAMOLEADMIN' AS username, 'GUACAMOLEADMIN' AS affected_username, 'UPDATE'     AS permission
    UNION SELECT 'GUACAMOLEADMIN' AS username, 'GUACAMOLEADMIN' AS affected_username, 'ADMINISTER' AS permission
) permissions
JOIN guacamole_entity          ON permissions.username = guacamole_entity.name AND guacamole_entity.type = 'USER'
JOIN guacamole_entity affected ON permissions.affected_username = affected.name AND guacamole_entity.type = 'USER'
JOIN guacamole_user            ON guacamole_user.entity_id = affected.entity_id;

SET @entityid = (select entity_id from guacamole_entity where name = 'GUACAMOLEADMIN'); 
