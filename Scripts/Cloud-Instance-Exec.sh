...
if [ "$CLOUD_OS" == "GCP_UBU" ]; then
	if [ "$THEACTION" == "A" ]; then
		...
		ssh $THEREQUSR@$THEREQMAC -p $THEREQPRT -o StrictHostKeyChecking=no -i "$THEREQPEM" "sudo touch /opt/EXEC1ON && sudo chmod 777 /opt/EXEC1ON && sudo mkdir -p /shiva && sudo chmod -R 777 /shiva && sudo mkdir -p /shiva/local && sudo chown -R matsya:matsya /shiva/local && sudo chmod -R u=rwx,g=,o= /shiva/local && sudo mkdir -p /shiva/local/storage && sudo chown -R matsya:matsya /shiva/local/storage && sudo chmod -R u=rwx,g=,o= /shiva/local/storage && sudo mkdir -p /shiva/global && sudo chown -R matsya:matsya /shiva/global && sudo chmod -R u=rwx,g=,o= /shiva/global && sudo mkdir -p /shiva/global/storage && sudo chown -R matsya:matsya /shiva/global/storage && sudo chmod -R u=rwx,g=,o= /shiva/global/storage && echo \"deb [signed-by=/usr/share/keyrings/cloud.google.asc] https://packages.cloud.google.com/apt gcsfuse-jammy main\" | sudo tee /etc/apt/sources.list.d/gcsfuse.list > /dev/null && curl https://packages.cloud.google.com/apt/doc/apt-key.gpg | sudo tee /usr/share/keyrings/cloud.google.asc && sudo apt-get update -y && sudo apt-get install gcsfuse -y && gcsfuse $THEREQBUC \"/shiva/local/storage\" && gcsfuse $THEREQGBK \"/shiva/global/storage\" && sudo mv /opt/EXEC1ON /opt/EXEC1DONE"
