...
deploy_instances_gcp() {
	...
	if [ "$CHOICE" == "A" ] ; then
		...
		sudo cp $BASE/tmp/$REALSECRETSFILE1NAME $BASE/tmp/.GCP-ServiceAccount
		...
		FILESTOBEDELETED+=("$BASE/tmp/.GCP-ServiceAccount")
		...
		sed -i -e s~"GCPSCOPE1VAL"~"$BASE/tmp/.GCP-ServiceAccount"~g $BASE/tmp/$GCPNAME.tf
