...
echo \"\"
	...
	do
		...
		sudo ssh vagrant@$VMIP -p 22  -o "StrictHostKeyChecking=no" -i "$BASE/Output/Pem/op-$CLUSTERNAME.pem" "echo \"$VMIP\" | sudo tee /opt/WHOAMI > /dev/null && sudo chmod 777 /opt/WHOAMI && sudo mkdir /shiva && sudo chmod -R 777 /shiva && sudo mkdir -p /shiva/local && sudo chown -R vagrant:vagrant /shiva/local && sudo chmod -R u=rwx,g=rwx,o=rwx /shiva/local && sudo mkdir -p /shiva/local/storage && sudo chown -R vagrant:vagrant /shiva/local/storage && sudo chmod -R u=rwx,g=rwx,o=rwx /shiva/local/storage && sudo mkdir -p /shiva/global && sudo chown -R vagrant:vagrant /shiva/global && sudo chmod -R u=rwx,g=rwx,o=rwx /shiva/global && sudo mkdir -p /shiva/global/storage && sudo chown -R vagrant:vagrant /shiva/global/storage && sudo chmod -R u=rwx,g=rwx,o=rwx /shiva/global/storage"
