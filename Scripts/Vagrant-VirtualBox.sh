...
echo \"\"
	...
	do
		...
		sudo ssh vagrant@$VMIP -p 22  -o "StrictHostKeyChecking=no" -i "$BASE/Output/Pem/op-$CLUSTERNAME.pem" "echo \"$VMIP\" | sudo tee /opt/WHOAMI > /dev/null && sudo chmod 777 /opt/WHOAMI && sudo touch /opt/VPC && sudo chmod 777 /opt/VPC && echo \"ONPREM\" | sudo tee -a /opt/VPC > /dev/null && sudo touch /opt/EXEC1ON && sudo chmod 777 /opt/EXEC1ON && sudo mkdir -p /shiva && sudo chmod -R 777 /shiva && sudo mkdir -p /shiva/bdd && sudo chown -R root:root /shiva/bdd && sudo chmod -R u=rwx,g=rwx,o=rwx /shiva/bdd && sudo mkdir -p /shiva/bdd/storage && sudo chown -R root:root /shiva/bdd/storage && sudo chmod -R u=rwx,g=rwx,o=rwx /shiva/bdd/storage && sudo mkdir -p /shiva/local && sudo chown -R root:root /shiva/local && sudo chmod -R u=rwx,g=rwx,o=rwx /shiva/local && sudo mkdir -p /shiva/local/storage && sudo chown -R root:root /shiva/local/storage && sudo chmod -R u=rwx,g=rwx,o=rwx /shiva/local/storage && sudo mkdir -p /shiva/local/bucket && sudo chown -R root:root /shiva/local/bucket && sudo chmod -R u=rwx,g=rwx,o=rwx /shiva/local/bucket && sudo mkdir -p /shiva/global && sudo chown -R root:root /shiva/global && sudo chmod -R u=rwx,g=rwx,o=rwx /shiva/global && sudo mkdir -p /shiva/global/bucket && sudo chown -R root:root /shiva/global/bucket && sudo chmod -R u=rwx,g=rwx,o=rwx /shiva/global/bucket && sudo ln -s /shiva/global/bucket /shiva/global/storage && sudo mv /opt/EXEC1ON /opt/EXEC1DONE"
		#sudo ssh vagrant@$VMIP -p 22  -o "StrictHostKeyChecking=no" -i "$BASE/Output/Pem/op-$CLUSTERNAME.pem" "sudo apt-get install -y curl openssl tar socat conntrack wget nano ebtables ipset chrony"
		#RANDOMUSERNAME="k8s"
		#RANDOMPASSWORD="$THEREALVISIONKEY"
		#USRWITHPWD="/opt/usrwithpwd"
		#sudo ssh vagrant@$VMIP -p 22  -o "StrictHostKeyChecking=no" -i "$BASE/Output/Pem/op-$CLUSTERNAME.pem" "sudo mkdir -p $USRWITHPWD && sudo useradd -d $USRWITHPWD/$RANDOMUSERNAME -s /bin/bash -m $RANDOMUSERNAME && sudo usermod -p \$(echo "$RANDOMPASSWORD" | openssl passwd -1 -stdin) $RANDOMUSERNAME && sudo usermod -aG sudo $RANDOMUSERNAME && sudo rm -f /etc/sudoers.d/$RANDOMUSERNAME-user && echo \"$RANDOMUSERNAME ALL=(ALL) NOPASSWD:ALL\" | sudo tee /etc/sudoers.d/$RANDOMUSERNAME-user > /dev/null && sudo mkdir -p $USRWITHPWD/$RANDOMUSERNAME/.ssh && sudo chown -R $RANDOMUSERNAME:$RANDOMUSERNAME $USRWITHPWD/$RANDOMUSERNAME/.ssh && sudo chmod -R u=rwx,g=rwx,o=rwx $USRWITHPWD/$RANDOMUSERNAME/.ssh && sudo chown -R $RANDOMUSERNAME:$RANDOMUSERNAME $USRWITHPWD/$RANDOMUSERNAME && sudo chmod -R u=rwx,g=rwx,o=rwx $USRWITHPWD/$RANDOMUSERNAME"
