...
echo -e "${ORANGE}=================================================================${NC}"
echo -e "${BLUE}${BOLD}\x1b[4mK${NORM}${NC}eystone ${BLUE}${BOLD}\x1b[4mR${NORM}${NC}emote ${BLUE}${BOLD}\x1b[4mI${NORM}${NC}ntelligent ${BLUE}${BOLD}\x1b[4mS${NORM}${NC}canning ${BLUE}${BOLD}\x1b[4mH${NORM}${NC}olistic ${BLUE}${BOLD}\x1b[4mN${NORM}${NC}etwork ${BOLD}\x1b[30;44mAssimilator\x1b[m${NORM}"
echo -e "${GREEN}=================================================================${NC}"
echo ''
...
# Function to display network interfaces and collect user choices
select_interfaces() {
...
# Function to collect user authentication information
collect_authentication() {
    while true; do
        ...
        username="${parts[0]}"
        # Removing ├ from the password
        password="${parts[1]//├/}"
        # Checking if password contains .pem to determine type
        if [[ "${parts[1]}" == *".pem" ]]; then
...
# Function to scan network for potential live hosts
scan_network() {
    for interface in "${Networks[@]}"; do
        # Get gateway IP address
        gateway=$(route -n | awk '$1 == "0.0.0.0" {print $2}' | head -n1)
        # Extract network portion from gateway IP
...
# Function to attempt SSH connections to potential hosts
# Function to attempt SSH connections to potential hosts
attempt_ssh() {
    ...
    for host in "${Potentials[@]}"; do
        for auth in "${Authentication[@]}"; do
            ...
            if [ "${parts[0]}" == "UP" ]; then
                ...
                # Using sshpass to automate SSH authentication with password
                if output=$(sshpass -p "$password" ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 "$username@$host" exit 2>&1); then
                    ssh_and_get_info=$(ssh_and_get_info "$host" "$username" "$password" "") && IFS='├' read -ra _gi <<< "$ssh_and_get_info" && _gi1="${_gi[1]}" && _gi2="${_gi[0]}"         
                    echo ",$host,$_gi1,$_gi2,$username,$password," >> $BASE/tmp/$NetworkScanResult.csv 
                    break          
                elif [[ "$output" == *"Permission denied"* ]]; then
                    if [[ "$username" == "root" ]]; then
                        #echo "$host,,,$username,$password,,,FAILURE,Permission denied" >> $BASE/tmp/$NetworkScanResult.csv
                        echo "$host,,,$username,$password,,,FAILURE,Permission denied"
                    else
                        echo "Trying SSH with username: root, password: $password, to host: $host..."
                        if sshpass -p "$password" ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 "root@$host" exit 2>&1 >/dev/null; then
                            ssh_and_get_info=$(ssh_and_get_info "$host" "root" "$password" "") && IFS='├' read -ra _gi <<< "$ssh_and_get_info" && _gi1="${_gi[1]}" && _gi2="${_gi[0]}"         
                            echo ",$host,$_gi1,$_gi2,$username,$password," >> $BASE/tmp/$NetworkScanResult.csv 
                            break                           
                        else
                            #echo "$host,,,$username,$password,,,NOSSH,Permission denied" >> $BASE/tmp/$NetworkScanResult.csv
                            echo "$host,,,$username,$password,,,NOSSH,Permission denied"
                        fi
                    fi
                else
                    #echo "$host,,,$username,$password,,,NOSSH,$output" >> $BASE/tmp/$NetworkScanResult.csv
                ...
                # Using regular ssh for PEM authentication
                if output=$(ssh -i "$pem_path" -o StrictHostKeyChecking=no -o ConnectTimeout=10 "$username@$host" exit 2>&1); then
                    ssh_and_get_info=$(ssh_and_get_info "$host" "$username" "" "$pem_path") && IFS='├' read -ra _gi <<< "$ssh_and_get_info" && _gi1="${_gi[1]}" && _gi2="${_gi[0]}"         
                    echo ",$host,$_gi1,$_gi2,$username,,$pem_path" >> $BASE/tmp/$NetworkScanResult.csv
                    break                    
                elif [[ "$output" == *"Permission denied"* ]]; then
                    if [[ "$username" == "root" ]]; then
                        #echo "$host,,,$username,,,$pem_path,FAILURE,Permission denied" >> $BASE/tmp/$NetworkScanResult.csv
                        echo "$host,,,$username,,,$pem_path,FAILURE,Permission denied"
                    else
                        echo "Trying SSH with username: root, PEM path: $pem_path, to host: $host..."
                        if ssh -i "$pem_path" -o StrictHostKeyChecking=no -o ConnectTimeout=10 "root@$host" exit 2>&1 >/dev/null; then
                            ssh_and_get_info=$(ssh_and_get_info "$host" "root" "" "$pem_path") && IFS='├' read -ra _gi <<< "$ssh_and_get_info" && _gi1="${_gi[1]}" && _gi2="${_gi[0]}"         
                            echo ",$host,$_gi1,$_gi2,$username,,$pem_path" >> $BASE/tmp/$NetworkScanResult.csv 
                            break                           
                        else
                            #echo "$host,,,$username,,,$pem_path,NOSSH,Permission denied" >> $BASE/tmp/$NetworkScanResult.csv
                            echo "$host,,,$username,,,$pem_path,NOSSH,Permission denied"
                        fi
                    fi
                else
                    #echo "$host,,,$username,,,$pem_path,NOSSH,$output" >> $BASE/tmp/$NetworkScanResult.csv
...
get_os_type() {
    ...
    if sshpass -p "$password" ssh -o StrictHostKeyChecking=no "$username@$host" 'cat /etc/debian_version' &>/dev/null; then
        echo "Debian"
    # Check if the system is Red Hat-based
    elif sshpass -p "$password" ssh -o StrictHostKeyChecking=no "$username@$host" 'cat /etc/redhat-release' &>/dev/null; then
        echo "RedHat"
    else
        echo "WinMac"
    fi
...
ssh_and_get_info() {
    ...
    if [ -n "$pem" ]; then
        os_type_and_hostname=$(sshpass -p "$password" ssh -o StrictHostKeyChecking=no -i "$pem" "$username@$host" \
        ...
        os_type_and_hostname=$(sshpass -p "$password" ssh -o StrictHostKeyChecking=no "$username@$host" \
            'os_type=$(if command -v apt-get &> /dev/null; then echo "Debian"; elif command -v yum &> /dev/null; then echo "RedHat"; else echo "WinMac"; fi); hostname=$(hostname); echo "$os_type├$hostname"')
    fi
...
#ssh_and_get_info "machine1" "username" "password" "/path/to/pem/file.pem"

# Example usage without PEM file:
#ssh_and_get_info "machine1" "username" "password" ""
...
# Run the main function with sudo
if [ "$(id -u)" != "0" ]; then
